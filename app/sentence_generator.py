...
from app.sentence_gen_statics import WORD_BLACKLIST, SPECIAL_WORDS, CONJUNCTIONS, PREPOSITIONS, BE
...
"""
    ...
    * Don't save pronouns, prepositions or conjunctions (handled by program).
    ...
    * English is full of exceptions. Some of these are handled by the program. For
    ...
    or add them to the WORD_BLACKLIST.
    ...
    * Sentence_generator aims to be able to generate grammatically correct sentences
    ...
    Sentence models consist of tags separated by forward slashes (/) where each 
    tag represents a word, e.g. "TAG/TAG/TAG/TAG/TAG". Each tag in turn consists
    of subtags, separated by periods (.) that represent properties of each word, 
    e.g. singular vs plural for nouns or whether a verb is regular or irregular.
    See each word class' word-getter-functions' doc strings or instructions on 
...
class Sentence:
    ...
    def get_verb(self, tag):
        ...
        if tags[1] != INFINITIVE:
            ...
            verb.word = self.conjugate(verb.word, verb.mult_syll, tags[1], verb.irregular)
    ...
    def conjugate(self, verb, mult_syll, tag, irregular):
        ...
        # Else either past tense, past continuous, present participle or 
        # past participle
        else:
            self.base_verb = verb
            ...
            # else it will be present participle, past participle or past continuous
            elif tag == PRESENT_PART:
                # Present participle is present tense of 'be' and the verb
                # in ing-form. 
                return self.conj_be(PRESENT_TENSE, subj_tags) + ' ' + verb + 'ing'
            
            elif tag == PAST_CONT:
                return self.conj_be(PAST_TENSE, subj_tags) + ' ' + verb + 'ing'
    ...
    def conj_be(self, form, subj_tag):
        """ Conjugate 'be'. checks sentence subject's word class and 
            count (singular/plural). Also checks desired form (tempus) of
            verb (be) and returns appropriate form. """
    ...
    def check_subj(self):
        """ Check if subject is a singular noun, proper noun or a pronoun 
    ...
    def check_do(self):
        """ Transform 'do' to 'does' if subject is a singular noun, proper
            noun or a pronoun in third person singular. """
    ...
    def check_have(self):
        """ Transform 'have' to 'has' if subject is a singular noun, proper
            noun or a pronoun in third person singular. """
    ...
    def get_card(self, tag):
        ...
        num = random.randint(2, int('9' * num_len) + 1)
